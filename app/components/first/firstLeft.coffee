...
template = '''
	<svg ng-init='vm.resize()' width='100%' ng-attr-height='{{vm.height + vm.mar.top + vm.mar.bottom}}'>
		...
		<g class='main' clip-path="url(#firstLeft)" shifter='{{[vm.mar.left, vm.mar.top]}}'>
...
class firstLeftCtrl extends Ctrl
	...
	mousemove: (event) =>
		...
		@point = _.findLast firstData.data, (d)=> (d.t <= @X.invert(event.x - rect.left))
		@scope.$evalAsync()
...
der = ()->
	directive = 
		...
		controller: ['$scope','$element', firstLeftCtrl]
